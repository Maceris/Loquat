...
namespace loquat
{
	...
	public:
		...
		LOQUAT_CPU_GPU
		Element get_mean() const noexcept
		{
			return mean;
		}
		...
		{
			return (count < 1 || mean == 0) ? 0 : variance() / get_mean();
		}
	...
	public:
		...
		template <typename F>
			requires requires(F f) { { f(x) }->std::convertible_to<T>; }
		LOQUAT_CPU_GPU
		...
		LOQUAT_CPU_GPU
		Float get_weight_sum() const noexcept
		{
			return weight_sum;
		}
		...
		std::string to_string() const noexcept
		{
			return std::format("[ WeightedReservoirSampler rng: {} ]"
				"weight_sum: {} reservoir: {} reservoir_weight: {}",
	...
	public:
		...
		std::string to_string() const noexcept
		{
			return std::format("[ PiecewiseConstant1D function: {} cdf: {} "
				"min: {} max: {} function_integral: {} ]", function,
		...
		pstd::optional<Float> invert(Float x) const noexcept
		{
			...
			int offset = clamp(static_cast<int>(c), 0, function.size() - 1);
	...
	public:
			...
			, marginal_density{ allocator }
		{
			...
			pstd::vector<Float> marginal_function;
		...
		std::string to_string() const noexcept
		{
			return std::format("[ PiecewiseConstant2D domain: {}"
				"conditional_densities: {} marginal_density: {} ]",
				m_domain.to_string(), conditional_densities,
				marginal_density.to_string());
		}
	...
	private:
		AABB2f m_domain;
		pstd::vector<PiecewiseConstant1D> conditional_densities;
