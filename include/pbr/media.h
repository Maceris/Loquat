...
namespace loquat
{
    ...
    public:
        ...
        LOQUAT_CPU_GPU
        pstd::optional<PhaseFunctionSample> sample_phase(Vec3f wo, Point2f u) const
        {
            Float pdf;
            Vec3f wi = sample_henyey_greenstein(wo, g, u, &pdf);
    ...
    struct MajorantGrid
    {
        ...
        MajorantGrid(AABB3f bounds, Point3i res, Allocator alloc)
            : bounds(bounds)
            , voxels(res.x* res.y* res.z, alloc)
            , res(res)
        {}
        ...
        {
            LOG_ASSERT(x >= 0 && x < res.x && y >= 0 && y < res.y && z >= 0 && z < res.z);
        ...
        LOQUAT_CPU_GPU
        AABB3f voxel_bounds(int x, int y, int z) const
        {
    ...
    public:
            ...
            , sigma_t(sigma_t)
        {
            ...
            for (int axis = 0; axis < 3; ++axis) {
                ...
                voxel[axis] =
                    clamp(grid_intersect[axis] * grid->res[axis], 0, grid->res[axis] - 1);
                deltaT[axis] = 1 / (std::abs(ray_grid.direction[axis]) * grid->res[axis]);
                if (ray_grid.direction[axis] == -0.0f)
                    ray_grid.direction[axis] = 0.0f;
    ...
    public:
        ...
        HomogeneousMedium(Spectrum sigma_a, Spectrum sigma_s, Float sigmaScale,
            Spectrum Le, Float LeScale, Float g, Allocator alloc)
            : sigma_a_spec(sigma_a, alloc)
            , sigma_s_spec(sigma_s, alloc)
            , emitted_spec(Le, alloc)
            , phase(g)
        {
            sigma_a_spec.scale(sigmaScale);
            sigma_s_spec.scale(sigmaScale);
            emitted_spec.scale(LeScale);
        }
        ...
        LOQUAT_CPU_GPU
        bool is_emissive() const { return emitted_spec.max_value() > 0; }
        ...
        MediumProperties sample_point(Point3f p, const SampledWavelengths& lambda) const
        {
            ...
            SampledSpectrum Le = emitted_spec.sample(lambda);
    ...
    private:
        ...
        DenselySampledSpectrum emitted_spec;
    ...
    public:
        ...
        GridMedium(const AABB3f& bounds, const Transform& renderFromMedium,
        ...
        LOQUAT_CPU_GPU
        bool is_emissive() const { return isEmissive; }
        ...
        MediumProperties sample_point(Point3f p, const SampledWavelengths& lambda) const
        {
            ...
            p = renderFromMedium.apply_inverse(p);
            ...
            if (isEmissive)
            {
                ...
                if (scale > 0)
                {
                    ...
                    {
                        Le = scale * emitted_spec.sample(lambda);
                    }
                }
            }
        ...
        LOQUAT_CPU_GPU
        DDAMajorantIterator sample_ray(Ray ray, Float raytMax,
            const SampledWavelengths& lambda) const
        {
    ...
    public:
        ...
        RGBGridMedium(const AABB3f& bounds, const Transform& renderFromMedium, Float g,
        ...
        LOQUAT_CPU_GPU
        bool is_emissive() const { return LeGrid && LeScale > 0; }
        ...
        MediumProperties sample_point(Point3f p, const SampledWavelengths& lambda) const
        {
            ...
            auto convert = [=] LOQUAT_CPU_GPU(RGBUnboundedSpectrum s) {
                return s.sample(lambda);
            };
            SampledSpectrum sigma_a =
                sigmaScale *
                (sigma_aGrid ? sigma_aGrid->lookup(p, convert) : SampledSpectrum(1.0f));
            SampledSpectrum sigma_s =
                sigmaScale *
                (sigma_sGrid ? sigma_sGrid->lookup(p, convert) : SampledSpectrum(1.0f));

            // Find emitted radiance _Le_ for _RGBGridMedium_
            SampledSpectrum Le(0.0f);
            if (LeGrid && LeScale > 0)
            {
                auto convert = [=] LOQUAT_CPU_GPU(RGBIlluminantSpectrum s) {
                    return s.sample(lambda);
                };
    ...
    private:
        ...
        Float LeScale;
        HGPhaseFunction phase;
        pstd::optional<SampledGrid<RGBUnboundedSpectrum>> sigma_aGrid;
        pstd::optional<SampledGrid<RGBUnboundedSpectrum>> sigma_sGrid;
        Float sigmaScale;
        MajorantGrid majorantGrid;
    };
    ...
    public:
        ...
        CloudMedium(const AABB3f& bounds, const Transform& renderFromMedium,
        ...
        LOQUAT_CPU_GPU
        bool is_emissive() const { return false; }
        ...
        MediumProperties sample_point(Point3f p, const SampledWavelengths& lambda) const
        {
            ...
            return MediumProperties{ sigma_a, sigma_s, &phase, SampledSpectrum(0.0f) };
        }

        LOQUAT_CPU_GPU
        HomogeneousMajorantIterator sample_ray(Ray ray, Float raytMax,
            const SampledWavelengths& lambda) const
        {
            // Transform ray to medium's space and compute bounds overlap
            ray = renderFromMedium.apply_inverse(ray, &raytMax);
    ...
    public:
        static inline void ptrAssert(void* ptr, const char* msg, const char* file, int line,
            ...
            if (abort)
            {
                LOG_FATAL(std::format("%p: {} ({}:{})", ptr, msg, file, line));
            }
            else
            {
                LOG_ERROR(std::format("%p: {} ({}:{})", ptr, msg, file, line));
            }
        }

        NanoVDBBuffer() = default;
        NanoVDBBuffer(Allocator alloc)
            : alloc(alloc)
        {}
        NanoVDBBuffer(size_t size, Allocator alloc = {})
            : alloc(alloc)
        {
            init(size);
        }
        ...
        {
            return NanoVDBBuffer(size, context ? context->get_allocator() : Allocator());
        }
    ...
    public:
        ...
        LOQUAT_CPU_GPU
        bool is_emissive() const { return temperatureFloatGrid && LeScale > 0; }
    ...
    LOQUAT_CPU_GPU inline Float PhaseFunction::phase(Vec3f wo, Vec3f wi) const
    {
        auto p = [&](auto ptr) { return ptr->phase(wo, wi); };
        return dispatch(p);
    }

    LOQUAT_CPU_GPU inline pstd::optional<PhaseFunctionSample> PhaseFunction::sample_phase(Vec3f wo,
        Point2f u) const
    {
        auto sample = [&](auto ptr) { return ptr->sample_phase(wo, u); };
        return dispatch(sample);
    }

    LOQUAT_CPU_GPU inline Float PhaseFunction::PDF(Vec3f wo, Vec3f wi) const
    {
        auto pdf = [&](auto ptr) { return ptr->PDF(wo, wi); };
        return dispatch(pdf);
    }

    LOQUAT_CPU_GPU inline pstd::optional<RayMajorantSegment> RayMajorantIterator::next()
    {
        auto next = [](auto ptr) { return ptr->next(); };
        return dispatch(next);
    }

    LOQUAT_CPU_GPU inline MediumProperties Medium::sample_point(Point3f p,
        const SampledWavelengths& lambda) const
    {
        auto sample = [&](auto ptr) { return ptr->sample_point(p, lambda); };
        return dispatch(sample);
    }
