...
namespace loquat
{
    ...
    namespace detail
    {
...
#ifdef LOQUAT_INT64_IS_OWN_TYPE
        template <>
        ...
        inline typename std::enable_if_t<std::is_class_v<typename std::decay_t<T>>, std::string>
            format_one(const char* fmt, T&& v)
        {
            LOG_FATAL(std::format("printf: Non-basic type {} passed for format string {}",
                typeid(v).name(), fmt));
            return "";
        }
        ...
        template <typename T, typename... Args>
        inline void string_printf_recursive_with_precision(std::string* s,
            const char* fmt, const std::string& nextFmt, T&& v, Args &&...args)
        {
            LOG_FATAL("");
        }
        ...
        inline typename std::enable_if_t<!std::is_class_v<typename std::decay_t<T>>, void>
            string_printf_recursive_with_precision(std::string* s, const char* fmt,
...
#pragma warning(pop)
    ...
    {
        std::string ret;
        detail::string_printf_recursive(&ret, fmt, std::forward<Args>(args)...);
        return ret;
    }
