...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        else if (tok == "soa")
        {
            ...
            {
                tok = getToken(false);
                soa.template_type = (std::string)tok;
                if (!isalpha(soa.template_type[0]))
                    error("%s: invalid type identifier.\n", soa.template_type.c_str());
                expect(">");
                expect("{");
            }
            else if (tok == ";")
            ...
            while (true)
            {
                ...
                // how to SOA pointers..
                if (member.pointer_count == 0 && member.type != soa.template_type &&
                ...
                while (true)
                {
                    ...
                    member.array_sizes.push_back("");  // assume no array for starters
                    ...
                    if (tok == "[")
                    {
                        ...
                        member.array_sizes[member.array_sizes.size() - 1] =
                            (std::string)tok;
                        expect("]");
                        tok = getToken(false);
                    }
    ...
    {
        if (!soa.template_type.empty())
        {
            printf("template <typename %s> struct SOA<%s<%s>> {\n",
                soa.template_type.c_str(), soa.type.c_str(), soa.template_type.c_str());
        }
        else
        {
        ...
        for (const auto& member : soa.members)
        {
            for (int i = 0; i < member.names.size(); ++i)
            {
                ...
                if (!member.array_sizes[i].empty())
                {
                    printf("        for (int i = 0; i < %s; ++i)\n",
                        member.array_sizes[i].c_str());
                    if (isFlatType(member.type) || member.pointer_count > 0)
                    {
                        printf(
                            "            this->%s[i] = alloc.allocate_object<%s>(n);\n",
                            name.c_str(), member.get_type().c_str());
                    }
                    else
                    {
                        assert(member.is_const == false && member.pointer_count == 0);
        ...
        if (!soa.template_type.empty())
        {
            printf("        LOQUAT_CPU_GPU\n");
            printf("        operator %s<%s>() const {\n", soa.type.c_str(),
                soa.template_type.c_str());
            printf("            %s<%s> r;\n", soa.type.c_str(), soa.template_type.c_str());
        }
        else
        {
            printf("        LOQUAT_CPU_GPU\n");
        ...
        for (const auto& member : soa.members)
        {
            for (int i = 0; i < member.names.size(); ++i)
            {
                ...
                {
                    if (isFlatType(member.type) || member.pointer_count > 0)
                        printf("    %s * /*LOQUAT_RESTRICT*/ %s[%s];\n",
                            member.get_type().c_str(), name.c_str(),
                            member.array_sizes[i].c_str());
                    else
                ...
                {
                    if (isFlatType(member.type) || member.pointer_count > 0)
                        printf("    %s * LOQUAT_RESTRICT %s;\n", member.get_type().c_str(),
                            name.c_str());
                    else
                        printf("    SOA<%s> %s;\n", member.type.c_str(), name.c_str());
                }
