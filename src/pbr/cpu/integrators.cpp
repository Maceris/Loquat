...
namespace loquat
{
	...
	{
		int max_depth = parameters.get_one_int("maxdepth", 5);
		return std::make_unique<RandomWalkIntegrator>(max_depth, camera,
			sampler, aggregate, lights);
	}

	std::string RandomWalkIntegrator::to_string() const
	{
		return std::format("[ RandomWalkIntegrator max_depth: {} ]", max_depth);
	}
    ...
    SimplePathIntegrator::SimplePathIntegrator(int max_depth, bool sample_lights,
        bool sample_BSDF, Camera camera,
        Sampler sampler, Primitive aggregate,
        std::vector<Light> lights)
        : RayIntegrator(camera, sampler, aggregate, lights),
        max_depth(max_depth),
        sample_lights(sample_lights),
        sample_BSDF(sample_BSDF),
        light_sampler(lights, Allocator())
    {}
    ...
    SampledSpectrum SimplePathIntegrator::Li(RayDifferential ray, SampledWavelengths& lambda,
        ...
        while (beta) {
            ...
            if (!si) {
                if (!sample_lights || specularBounce)
            ...
            if (sample_lights) {
                pstd::optional<SampledLight> sampledLight =
                    light_sampler.Sample(sampler.Get1D());
                if (sampledLight) {
    ...
    std::string SimplePathIntegrator::ToString() const {
        return StringPrintf("[ SimplePathIntegrator max_depth: %d sample_lights: %s "
            "sample_BSDF: %s ]",
            max_depth, sample_lights, sample_BSDF);
    }

    std::unique_ptr<SimplePathIntegrator> SimplePathIntegrator::create(
        const ParameterDictionary& parameters, Camera camera, Sampler sampler,
        Primitive aggregate, std::vector<Light> lights, const FileLoc* loc) {
        int max_depth = parameters.GetOneInt("maxdepth", 5);
        bool sample_lights = parameters.GetOneBool("samplelights", true);
        bool sample_BSDF = parameters.GetOneBool("samplebsdf", true);
        return std::make_unique<SimplePathIntegrator>(max_depth, sample_lights, sample_BSDF,
            camera, sampler, aggregate, lights);
    }

    // LightPathIntegrator Method Definitions
    LightPathIntegrator::LightPathIntegrator(int max_depth, Camera camera, Sampler sampler,
    ...
    void LightPathIntegrator::EvaluatePixelSample(Point2i pPixel, int sampleIndex,
        ...
        pstd::optional<SampledLight> sampledLight = light_sampler.Sample(ul);
        if (!sampledLight)
            return;
    ...
    std::string LightPathIntegrator::ToString() const {
        return StringPrintf("[ LightPathIntegrator max_depth: %d light_sampler: %s ]", max_depth,
            light_sampler);
    }
    ...
    std::unique_ptr<LightPathIntegrator> LightPathIntegrator::create(
        ...
        int max_depth = parameters.GetOneInt("maxdepth", 5);
        return std::make_unique<LightPathIntegrator>(max_depth, camera, sampler, aggregate,
            lights);
    }
    ...
    PathIntegrator::PathIntegrator(int max_depth, Camera camera, Sampler sampler,
        Primitive aggregate, std::vector<Light> lights,
        const std::string& lightSampleStrategy, bool regularize)
        : RayIntegrator(camera, sampler, aggregate, lights),
        max_depth(max_depth),
        light_sampler(LightSampler::create(lightSampleStrategy, lights, Allocator())),
        ...
        while (true) {
            ...
            if (!si) {
                ...
                for (const auto& light : infiniteLights) {
                    ...
                    if (depth == 0 || specularBounce)
                        ...
                        Float p_l = light_sampler.PMF(prevIntrCtx, light) *
            ...
            if (Le) {
                if (depth == 0 || specularBounce)
                    ...
                    Float p_l = light_sampler.PMF(prevIntrCtx, areaLight) *
    ...
    SampledSpectrum PathIntegrator::SampleLd(const SurfaceInteraction& intr, const BSDF* bsdf,
        ...
        Float u = sampler.Get1D();
        pstd::optional<SampledLight> sampledLight = light_sampler.Sample(ctx, u);
        Point2f uLight = sampler.Get2D();
        if (!sampledLight)
            return {};
    ...
    std::string PathIntegrator::ToString() const {
        return StringPrintf("[ PathIntegrator max_depth: %d light_sampler: %s regularize: %s ]",
            max_depth, light_sampler, regularize);
    }
    ...
    std::unique_ptr<PathIntegrator> PathIntegrator::create(
        ...
        return std::make_unique<PathIntegrator>(max_depth, camera, sampler, aggregate, lights,
            lightStrategy, regularize);
    }

    // SimpleVolumePathIntegrator Method Definitions
    SimpleVolumePathIntegrator::SimpleVolumePathIntegrator(int max_depth, Camera camera,
        Sampler sampler, Primitive aggregate,
        std::vector<Light> lights)
        : RayIntegrator(camera, sampler, aggregate, lights), max_depth(max_depth) {
        for (Light light : lights) {
            if (IsDeltaLight(light.Type()))
                ErrorExit("SimpleVolumePathIntegrator only supports area and infinite light "
                    "sources");
        }
    }

    SampledSpectrum SimpleVolumePathIntegrator::Li(RayDifferential ray,
        ...
        while (true) {
            ...
            if (!bsdf)
                ...
                if (bsdf.Sample_f(-ray.d, uc, u))
                    ErrorExit("SimpleVolumePathIntegrator doesn't support surface scattering.");
                else
                    break;
            }
        }
    ...
    std::string SimpleVolumePathIntegrator::ToString() const {
        return StringPrintf("[ SimpleVolumePathIntegrator max_depth: %d ] ", max_depth);
    }

    std::unique_ptr<SimpleVolumePathIntegrator> SimpleVolumePathIntegrator::create(
        const ParameterDictionary& parameters, Camera camera, Sampler sampler,
        Primitive aggregate, std::vector<Light> lights, const FileLoc* loc) {
        int max_depth = parameters.GetOneInt("maxdepth", 5);
        return std::make_unique<SimpleVolumePathIntegrator>(max_depth, camera, sampler, aggregate,
            lights);
    }
    ...
    SampledSpectrum VolumePathIntegrator::Li(RayDifferential ray, SampledWavelengths& lambda,
        Sampler sampler, ScratchBuffer& scratchBuffer,
        VisibleSurface* visibleSurf) const {
        ...
        while (true) {
                ...
                for (const auto& light : infiniteLights) {
                    if (SampledSpectrum Le = light.Le(ray, lambda); Le) {
                        if (depth == 0 || specularBounce)
                            ...
                            Float p_l = light_sampler.PMF(prevIntrContext, light) *
            ...
            if (SampledSpectrum Le = isect.Le(-ray.d, lambda); Le) {
                ...
                if (depth == 0 || specularBounce)
                    ...
                    Float p_l = light_sampler.PMF(prevIntrContext, areaLight) *
    ...
    SampledSpectrum VolumePathIntegrator::SampleLd(const Interaction& intr, const BSDF* bsdf,
        SampledWavelengths& lambda, Sampler sampler,
        SampledSpectrum beta,
    ...
    std::string VolumePathIntegrator::ToString() const {
        return StringPrintf(
            "[ VolumePathIntegrator max_depth: %d light_sampler: %s regularize: %s ]", max_depth,
            light_sampler, regularize);
    }

    std::unique_ptr<VolumePathIntegrator> VolumePathIntegrator::create(
        const ParameterDictionary& parameters, Camera camera, Sampler sampler,
        Primitive aggregate, std::vector<Light> lights, const FileLoc* loc) {
        int max_depth = parameters.GetOneInt("maxdepth", 5);
        std::string lightStrategy = parameters.GetOneString("lightsampler", "bvh");
        bool regularize = parameters.GetOneBool("regularize", false);
        return std::make_unique<VolumePathIntegrator>(max_depth, camera, sampler, aggregate,
    ...
    int RandomWalk(const Integrator& integrator, SampledWavelengths& lambda,
        ...
        int max_depth, TransportMode mode, Vertex* path, bool regularize);
    ...
    struct Vertex {
        ...
        Float PDFLightOrigin(const std::vector<Light>& infiniteLights, const Vertex& v,
            ...
            if (IsInfiniteLight()) {
                ...
                return InfiniteLightDensity(infiniteLights, light_sampler, w);

            }
            else {
                // Return sampling density for noninfinite light source
                Light light = (type == VertexType::Light) ? ei.light : si.areaLight;
                Float pdfPos, pdfDir, pdfChoice = light_sampler.PMF(light);
                if (IsOnSurface())
                    light.PDF_Le(ei, w, &pdfPos, &pdfDir);
                else
    ...
    int GenerateCameraSubpath(const Integrator& integrator, const RayDifferential& ray,
        SampledWavelengths& lambda, Sampler sampler,
        ScratchBuffer& scratchBuffer, int max_depth, Camera camera,
        ...
        return RandomWalk(integrator, lambda, ray, sampler, camera, scratchBuffer, beta,
            pdfDir, max_depth - 1, TransportMode::Radiance, path + 1,
            regularize) +
            1;
    }
    ...
    int GenerateLightSubpath(const Integrator& integrator, SampledWavelengths& lambda,
        ...
        int max_depth, Float time, LightSampler light_sampler,
        Vertex* path, bool regularize) {
        ...
        pstd::optional<SampledLight> sampledLight = light_sampler.Sample(sampler.Get1D());
        if (!sampledLight)
            return 0;
        Light light = sampledLight->light;
        ...
        int nVertices = RandomWalk(integrator, lambda, ray, sampler, camera, scratchBuffer,
            beta, les->pdfDir, max_depth - 1, TransportMode::Importance,
            path + 1, regularize);
        ...
        if (path[0].IsInfiniteLight()) {
            ...
            path[0].pdfFwd =
                InfiniteLightDensity(integrator.infiniteLights, light_sampler, ray.d);
        }
    ...
    Float InfiniteLightDensity(const std::vector<Light>& infiniteLights,
        ...
        Float pdf = 0;
        for (const auto& light : infiniteLights)
            pdf += light.PDF_Li(Interaction(), -w) * light_sampler.PMF(light);
        return pdf;
    }
    ...
    void BDPTIntegrator::Render() {
        ...
        if (visualizeStrategies || visualizeWeights) {
            const int bufferCount = (1 + max_depth) * (6 + max_depth) / 2;
    ...
    SampledSpectrum BDPTIntegrator::Li(RayDifferential ray, SampledWavelengths& lambda,
        ...
        Vertex* cameraVertices = scratchBuffer.Alloc<Vertex[]>(max_depth + 2);
        int nCamera = GenerateCameraSubpath(*this, ray, lambda, sampler, scratchBuffer,
            max_depth + 2, camera, cameraVertices, regularize);
        Vertex* lightVertices = scratchBuffer.Alloc<Vertex[]>(max_depth + 1);
        int nLight = GenerateLightSubpath(*this, lambda, sampler, camera, scratchBuffer,
            max_depth + 1, cameraVertices[0].time(),
            light_sampler, lightVertices, regularize);
            ...
            for (int s = 0; s <= nLight; ++s) {
                ...
                SampledSpectrum Lpath =
                    ConnectBDPT(*this, lambda, lightVertices, cameraVertices, s, t,
                        light_sampler, camera, sampler, &pFilmNew, &misWeight);
                PBRT_DBG("%s\n",
    ...
    std::string BDPTIntegrator::ToString() const {
        return StringPrintf("[ BDPTIntegrator max_depth: %d visualizeStrategies: %s "
            "visualizeWeights: %s regularize: %s light_sampler: %s ]",
            max_depth, visualizeStrategies, visualizeWeights, regularize,
            light_sampler);
    }
    ...
    std::unique_ptr<BDPTIntegrator> BDPTIntegrator::create(
        ...
        if ((visualizeStrategies || visualizeWeights) && max_depth > 5) {
        ...
        bool regularize = parameters.GetOneBool("regularize", false);
        return std::make_unique<BDPTIntegrator>(camera, sampler, aggregate, lights, max_depth,
    ...
    SampledSpectrum MLTIntegrator::L(ScratchBuffer& scratchBuffer, MLTSampler& sampler,
        ...
        *pRaster = sampleBounds.Lerp(sampler.GetPixel2D());
        ...
        if (GenerateLightSubpath(*this, *lambda, &sampler, camera, scratchBuffer, s,
            cameraVertices[0].time(), light_sampler, lightVertices,
            regularize) != s)
            return SampledSpectrum(0.f);
        ...
        SampledSpectrum L = ConnectBDPT(*this, *lambda, lightVertices, cameraVertices, s, t,
            light_sampler, camera, &sampler, &pRasterNew) *
            nStrategies;
        if (pRasterNew)
            *pRaster = *pRasterNew;
        return L;
    ...
    void MLTIntegrator::Render() {
        ...
        Timer timer;
        int nBootstrapSamples = nBootstrap * (max_depth + 1);
        ...
        ParallelFor(0, nBootstrap, [&](int64_t start, int64_t end) {
            ...
            for (int64_t i = start; i < end; ++i) {
                ...
                for (int depth = 0; depth <= max_depth; ++depth) {
                    int rngIndex = i * (max_depth + 1) + depth;
        ...
        Float b = Float(max_depth + 1) / bootstrapWeights.size() *
    ...
    std::string MLTIntegrator::ToString() const {
        return StringPrintf("[ MLTIntegrator camera: %s max_depth: %d nBootstrap: %d "
            "nChains: %d mutationsPerPixel: %d sigma: %f "
            "largeStepProbability: %f light_sampler: %s regularize: %s ]",
            camera, max_depth, nBootstrap, nChains, mutationsPerPixel, sigma,
            largeStepProbability, light_sampler, regularize);
    }
    ...
    std::unique_ptr<MLTIntegrator> MLTIntegrator::create(
        ...
        bool regularize = parameters.GetOneBool("regularize", false);
        return std::make_unique<MLTIntegrator>(camera, aggregate, lights, max_depth,
    ...
    void SPPMIntegrator::Render() {
        ...
        BVHLightSampler light_sampler(lights, Allocator());
            ...
            ParallelFor2D(pixelBounds, [&](Bounds2i tileBounds) {
                ...
                for (Point2i pPixel : tileBounds) {
                    ...
                    while (true) {
                        ...
                        if (depth++ == max_depth || haveSetVisiblePoint)
                            break;

                        // Accumulate direct illumination at SPPM camera ray intersection
                        SampledSpectrum Ld =
                            SampleLd(isect, bsdf, lambda, sampler, &light_sampler);
                        if (Ld)
                            pixel.Ld += film.ToOutputRGB(beta * Ld, lambda);

                        // Possibly create visible point and end camera path
                        BxDFFlags flags = bsdf.Flags();
                        if (IsDiffuse(flags) || (IsGlossy(flags) && depth == max_depth)) {
    ...
    std::string SPPMIntegrator::ToString() const {
        return StringPrintf("[ SPPMIntegrator camera: %s initialSearchRadius: %f "
            "max_depth: %d photonsPerIteration: %d "
            "colorSpace: %s digitPermutations: (elided) ]",
            camera, initialSearchRadius, max_depth, photonsPerIteration,
            *colorSpace);
    }
    ...
    std::unique_ptr<SPPMIntegrator> SPPMIntegrator::create(
        ...
        return std::make_unique<SPPMIntegrator>(camera, sampler, aggregate, lights,
            photonsPerIter, max_depth, radius, seed,
            colorSpace);
    }
    ...
    std::unique_ptr<Integrator> Integrator::create(
        ...
        if (name == "path")
            ...
            integrator = SimpleVolumePathIntegrator::create(parameters, camera, sampler,
                aggregate, lights, loc);
        else if (name == "volpath")
            integrator = VolumePathIntegrator::create(parameters, camera, sampler, aggregate,
                lights, loc);
        else if (name == "bdpt")
            integrator =
