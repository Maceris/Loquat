...
namespace loquat
{
	...
	void create_frame_buffers() noexcept
	{
		...
		g_global_state->pipeline->frame_buffers.resize(image_views.size());
		for (size_t i = 0; i < image_views.size(); ++i)
		{
			...
			if (vkCreateFramebuffer(device, &frame_buffer_info, nullptr,
				&g_global_state->pipeline->frame_buffers[i]) != VK_SUCCESS)
			{
	...
	void destroy_frame_buffers() noexcept
	{
		...
		LOG_ASSERT(g_global_state->pipeline != nullptr 
			&& "Cleaning up buffers for a null pipeline");

		for (auto framebuffer : g_global_state->pipeline->frame_buffers)
		{
